...
describe('CHASE1: the HUD names the quarry', () => {
  ...
  const model = (chase: HudModel['chase']): HudModel => ({
    active: character(), roster: [character()], abilities: [], modes: [], rotations: [], catalysts: [],
  ...
  const handlers = () => ({
    ...
    hold: vi.fn(), lock: vi.fn(), toggleProjection: vi.fn(), toggleOrbit: vi.fn(), recentre: vi.fn(), extendTime: vi.fn(),
    selectMode: vi.fn(),
