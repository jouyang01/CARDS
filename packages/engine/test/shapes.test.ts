...
describe('CONE-B: rotating a cone does not change its size', () => {
  ...
  it('the axis-aligned footprint is exactly what it was before CONE-B', () => {
    ...
    for (const [dir, step] of [[{ x: 1, y: 0 }, 0], [{ x: 0, y: 1 }, AIM_STEPS / 4], [{ x: -1, y: 0 }, AIM_STEPS / 2]] as const) {
...
describe('HITBOX1: cross-engine determinism', () => {
  ...
  it('every shape, every aim step, every range folds to a fixed value', () => {
    // Regenerated by AIM-SMOOTH (AIM_STEPS 256 → 512). The value **must** change
    // when the step count does — twice as many aims are folded in — so a bump
    // that left it alone would mean the fold was not reading the constant.
    // Was -1082420298 at 256.
    expect(signature()).toBe(462721398);
  });
  ...
  it('pins one aim exactly, so a signature change can be read', () => {
    // A bare signature number tells you *that* the geometry moved and nothing
    // about how. This is the readable half: one genuinely rotated aim —
    // five-eighths of a quadrant round from east, south-south-east — spelled
    // out tile by tile. Expressed against AIM_STEPS so it keeps meaning the
    // same direction after a resolution bump.
    const b = openBoard(41);
    const sse = Math.round((AIM_STEPS / 4) * 0.625);
    expect(posKeys(coneSquares(b, { x: 20, y: 20 }, stepToVector(sse), 3))).toEqual([
