...
/**
 * A pre-placed trap. `expiresOnTurn` is deliberately far out: these cases are
 * about triggering, not about TRAP-LIFETIME's sweep, and a trap that quietly
 * expired mid-test would fail for the wrong reason. Expiry has its own file.
 */
const trapAt = (x: number, y: number, owner: TeamId, expiresOnTurn = 99): TrapState => ({ id: `pre-${x}-${y}`, owner, ownerUnitId: `pre-owner-${owner}`, abilityId: 'trap', pos: { x, y }, damage: 20, expiresOnTurn, onTrigger: [{ kind: 'reveal', duration: 2 }] });
